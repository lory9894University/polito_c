...
players playerListInit() {
    players playersList = malloc(sizeof(*playersList));
...
copysize(link currentCharacter, char *name, char *class, char *id, int hp, int mp, int atk, int def, int mag, int spr) {
    ...
    currentCharacter->gearEquipped = (item **) malloc(
            sizeof(item * ) * 8); //quando poi servirà verrà allocato dinamicamente
}
...
void statsCalculation(players *playerslist, char *playerCode) {
    ...
    printf("id:%s nome:%s classe:%s\n-ha le seguenti statistiche:\n\thp:%d mp:%d atk:%d def:%d mag:%d spr:%d\n", x->id,
           x->name, x->class, Ihp, Imp, Iatk, Idef, Imag, Ispr);
}
...
void itemEquip(players *playerslist, char *playerCode, char *itemName, equipableItem itemWrapper) {
    ...
    if (itemSearchEquip(itemName, x->gearEquipped, x->gearInUse) != -1) { //ricerco l'oggetto nell'inventario
        printf("oggetto gia' equipaggiato\n");
        return;
    }
    if ((itemPosition = itemSearchInventary(itemName, itemWrapper)) != -1) {
        x->gearEquipped[x->gearInUse] = indexToPointer(itemWrapper, itemPosition);
        x->gearInUse++;
...
void itemUnequip(players *playerslist, char *playerCode, char *itemName) {
    ...
    if ((itemPosition = itemSearchEquip(itemName, x->gearEquipped, x->gearInUse)) == -1) {
