...
void transacAcq(FILE *fPtr, int transactionNrTmp,stock x,int memorized,int S){
    int j;
    int year, month, day;
...
stockList stockListCreation(char *filename,int S) {
    ...
    for (i = 0; i < listWrapper->stockNr; i++) {
        ...
        transacAcq(fPtr,transactionNrTmp,x,0,S);
    }
    fclose(fPtr);
    return listWrapper;
}
...
void stockListAdd(stockList list,char *filename,int S){
    FILE *fPtr;
    ...
    for (i = 0; i <stockNr ; i++) {
        ...
        if ((newStock=stockSearch(stockName,list))==NULL) {
            ...
            transacAcq(fPtr,transactionNrTmp,newStock,0,S);
        }
        else{
            transacAcq(fPtr,transactionNrTmp,newStock,1,S);
        }
    }
    fclose(fPtr);
}
