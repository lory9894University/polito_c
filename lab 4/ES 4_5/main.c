...
enum comando_e {ord_data, ord_tratta,ord_partenza,ord_arrivo,s_partenza, stop, error};
void stampa (automezzi *riga[], char row[],int nr); //stampa su file o su schermo
int operazione (automezzi *p_riga[], automezzi *a_riga[], automezzi *t_riga[], automezzi *d_riga[], enum comando_e comando, char *row, int nr);
void copia(automezzi riga[], int *nr);
enum comando_e menu (char row[]); //funzione che permette all'utente di effettuare una scelta
...
int main() {
    automezzi riga[1000];
    automezzi *p_riga[1000],*a_riga[1000],*t_riga[1000], *d_riga[1000], *swap;
    ...
    copia(riga,&nr);
    /*creazione di 4 vettori di puntatori al vettore di struct*/
    for (int i = 0; i <nr ; i++) {
    ...
    while (!end) {
        ...
        end=operazione(p_riga,a_riga,t_riga,d_riga,comando,row,nr);
    }
...
int operazione (automezzi *p_riga[], automezzi *a_riga[], automezzi *t_riga[], automezzi *d_riga[], enum comando_e comando, char *row, int nr){
    ...
    int i, l,r; //i contatore e indice della mia ricerca dicotomica

    switch (comando){
        /*i vettori di puntatori sono già tutti ordinati, perciò alla chiamata dell'utente corrisponde la stampa*/
        case ord_partenza:
