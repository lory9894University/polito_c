...
typedef enum {newPlayerList=0, newPlayer,deletePlayer ,newItemList, equipItem, unequipItem,emptyMem,stats,help,printPLayer,error,end} choice;
...
int main() {
    ...
    itemInit(&itemWrapper); //TODO, la funzione restituisce void,in futuro dovrà restituire puntatre a itemWraper
#if dbg
    setbuf(stdout,0);
#endif
    playerListInit(&playersList);
    showHelp();
...
choice menu(char inputLine[],players playersList,equipableItem itemWrapper){
    ...
    if((!strcmp(userChoice,"statistiche")||!strcmp(userChoice,"rimuovi")||!strcmp(userChoice,"aggiungi")||!strcmp(userChoice,"visualizza")) && (emptyItem(itemWrapper) || emptyPg(playersList)))
        return emptyMem;
